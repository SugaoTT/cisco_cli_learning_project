...
外観は私が学部4年時に開発したシステムのものを踏襲しており、工夫したこととしては、より便利に利用できるように、様々なフレームワークを取り入れており、CytoScape.jsを用いたグラフ作画や、xterm.jsを用いたターミナルの作成等を実施した。
苦労した点としては、スクリプト部分をTypeScriptで記述しており、javascript用のフレームワークをどのように組み込んでいくかについて、理解するまでに時間がかかった。
